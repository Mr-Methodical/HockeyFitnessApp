    ...
    return (
      <View style={[
        ...
        {isGroupChat && !isOwnMessage && (
          <Text style={[styles.senderName, { color: currentTheme.textSecondary }]}>{item.senderName}</Text>
        )}
        <View style={[
          styles.messageBubble,
          isOwnMessage 
            ? { ...styles.ownMessageBubble, backgroundColor: currentTheme.primary } 
            : { ...styles.otherMessageBubble, backgroundColor: currentTheme.surface, borderColor: currentTheme.border, borderWidth: 1 }
        ]}>
          <Text style={[
            styles.messageText,
            isOwnMessage 
              ? { ...styles.ownMessageText, color: currentTheme.surface } 
              : { ...styles.otherMessageText, color: currentTheme.text }
          ]}>
            {item.text}
          </Text>
          <Text style={[
            styles.messageTime,
            isOwnMessage 
              ? { ...styles.ownMessageTime, color: currentTheme.surface + '70' } 
              : { ...styles.otherMessageTime, color: currentTheme.textSecondary }
          ]}>
    ...
    >
      <SafeAreaView style={[styles.container, { backgroundColor: currentTheme.background }]}>
          ...
          >
            <View style={styles.headerTextContainer}>
              <Text style={[styles.headerTitle, { color: currentTheme.text }]}>
                ...
                {isGroupChat && <MaterialIcons name="edit" size={16} color={currentTheme.textSecondary} style={styles.editIcon} />}
              </Text>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContent, { backgroundColor: currentTheme.surface }]}>
              <Text style={[styles.modalTitle, { color: currentTheme.text }]}>Edit Group Name</Text>
              
              <TextInput
                style={[styles.modalInput, { borderColor: currentTheme.border, color: currentTheme.text, backgroundColor: currentTheme.inputBackground }]}
                value={tempGroupName}
        ...
        <FlatList
          ...
          style={[styles.messagesList, { backgroundColor: currentTheme.background }]}
        ...
        <View style={[styles.inputContainer, { backgroundColor: currentTheme.surface, borderTopColor: currentTheme.border }]}>
          <TextInput
            style={[styles.messageInput, { borderColor: currentTheme.inputBorder, color: currentTheme.inputText, backgroundColor: currentTheme.inputBackground }]}
            value={newMessage}
          ...
          <TouchableOpacity
            style={[
              styles.sendButton,
              { backgroundColor: (!newMessage.trim() || sending || (userProfile?.role === 'player' && messages.length === 0)) ? currentTheme.border : currentTheme.primary }
            ]}
...
const styles = StyleSheet.create({
  ...
  sendButtonDisabled: {}, // Using dynamic styling
  headerTextContainer: {
    alignItems: 'center',
  },
  editIcon: {
  ...
  cancelButton: {
    backgroundColor: '#f0f0f0', // Keep this neutral gray for cancel button
  },
  saveButton: {}, // Using dynamic styling
  cancelButtonText: {
    fontSize: 16,
    fontWeight: '600',
  },
