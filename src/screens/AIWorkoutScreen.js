...
import { saveAIWorkout } from '../services/workoutStorage';
import { getWorkoutSuggestions, generatePreStoredWorkout, generateWorkoutInstant } from '../services/preStoredWorkouts';
import { generateWorkoutImageFast, generateImageInBackground, getWorkoutImage } from '../services/preStoredImages';
  ...
  */
  ...
  const selectImage = async () => {
    try {
      ...
      WebCompatibleAlert.alert('Error', 'Failed to select image');
    }
  };
  ...
  const takePhoto = async () => {
    try {
      ...
      if (status !== 'granted') {
        WebCompatibleAlert.alert('Permission Required', 'Camera permission is needed to take photos');
        return;
      }
      ...
      WebCompatibleAlert.alert('Error', 'Failed to take photo');
    }
  };

  const loadSuggestions = async () => {
    try {
      const workoutSuggestions = getWorkoutSuggestions(user.uid, userProfile?.role);
  ...
  const generateNewWorkout = async (customPreferences = null) => {
    ...
    try {
      ...
      // Generate workout from pre-stored data - instant and consistent
      console.log('🚀 Using pre-stored workout generation (instant)');
      const generatedWorkout = generatePreStoredWorkout(workoutPrefs, userProfile?.role);
      ...
      WebCompatibleAlert.alert('Error', 'Failed to generate workout. Please try again.');
    } finally {
      setLoading(false);
  ...
  const showImageOptionsForCompletion = () => {
    ...
    if (Platform.OS === 'web') {
      // For web, use simpler approach
      if (isImageRequired) {
        const choice = window.confirm('Your team requires an image. Would you like to choose a file or use AI generated image?\n\nClick OK for file upload, Cancel for AI generated');
        if (choice) {
          selectImageForCompletion();
        } else {
          setSelectedImage(null);
          saveWorkoutSession(false, 'generate-ai');
        }
      } else {
        const choice = window.confirm('Would you like to add an image to your workout?\n\nClick OK to upload image, Cancel to skip');
        if (choice) {
          selectImageForCompletion();
        } else {
          saveWorkoutSession(true); // Skip image
        }
      }
    } else {
      // Use native Alert for mobile
  ...
  const selectImageForCompletion = async () => {
    try {
      if (Platform.OS === 'web') {
        ...
        input.onchange = (e) => {
          ...
          if (file) {
            ...
            reader.onload = (e) => {
              ...
              saveWorkoutSession(false, e.target.result);
            };
            reader.readAsDataURL(file);
          } else {
            saveWorkoutSession(false, null);
          }
        };
        input.click();
      } else {
        // Mobile image picker code (existing)
        const result = await ImagePicker.launchImageLibraryAsync({
          mediaTypes: ImagePicker.MediaTypeOptions.Images,
          allowsEditing: true,
          aspect: [16, 9],
        ...
        saveWorkoutSession(false, result.canceled || !result.assets[0] ? null : result.assets[0].uri);
      }
    } catch (error) {
  ...
  const saveWorkoutSession = async (skipImage = false, userImageUri = null) => {
    ...
    try {
      ...
      if (skipImage || preferences.imageSpeed === 'none') {
        ...
        try {
          if (preferences.imageSpeed === 'instant') {
            ...
            console.log('🎯 Using detailed image generation with pre-stored images');
            // Detailed mode - use pre-stored image approach
            imageToSave = await getWorkoutImage(
              null, // workoutId not needed for pre-stored approach
              workout.title,
              workout.description,
              user.uid,
              userProfile?.role
      ...
      WebCompatibleAlert.alert('Error', 'Failed to save workout. Please try again.');
  ...
  const renderExercise = (exercise, section, key) => (
    <TouchableOpacity
      ...
      style={[
        ...
        exercise.completed && { backgroundColor: currentTheme.primaryLight }
      ]}
    ...
    >
      <View style={styles.exerciseHeader}>
        <Ionicons
          ...
          size={24}
          color={exercise.completed ? currentTheme.success : currentTheme.textSecondary}
        />
        <Text style={[
          styles.exerciseName,
          { color: currentTheme.text },
          exercise.completed && { color: currentTheme.primary }
        ]}>
          {exercise.exercise}
        </Text>
      </View>
      ...
      <View style={styles.exerciseDetails}>
        {exercise.sets && (
          <Text style={[styles.exerciseInfo, { color: currentTheme.textSecondary }]}>
            {exercise.sets} sets × {exercise.reps}
          </Text>
        )}
        {exercise.duration && (
          <Text style={[styles.exerciseInfo, { color: currentTheme.textSecondary }]}>Duration: {exercise.duration}</Text>
        )}
        {exercise.rest && (
          <Text style={[styles.exerciseInfo, { color: currentTheme.textSecondary }]}>Rest: {exercise.rest}</Text>
        )}
      </View>
      
      <Text style={[styles.exerciseDescription, { color: currentTheme.textSecondary }]}>{exercise.description}</Text>
      
      {exercise.tips && (
        <Text style={[styles.exerciseTips, { color: currentTheme.accent }]}>💡 {exercise.tips}</Text>
      )}
    </TouchableOpacity>
  );
  ...
  const renderSection = (title, exercises, section, icon) => (
    <View style={[styles.section, { backgroundColor: currentTheme.surface }]}>
      <View style={styles.sectionHeader}>
        ...
        <Text style={[styles.sectionProgress, { color: currentTheme.textSecondary }]}>
  ...
  const ImageOptionsModal = () => (
    ...
    >
      <View style={styles.imageModalOverlay}>
        <View style={[styles.imageModalContainer, { backgroundColor: currentTheme.surface }]}>
          <View style={styles.imageModalHeader}>
            <Text style={[styles.imageModalTitle, { color: currentTheme.text }]}>Workout Image</Text>
            <TouchableOpacity onPress={() => setShowImageOptions(false)}>
              <Ionicons name="close" size={24} color={currentTheme.textSecondary} />
            </TouchableOpacity>
          </View>
          
          <Text style={[styles.imageModalSubtext, { color: currentTheme.textSecondary }]}>
            Choose how to add an image to your workout
          </Text>
          
          <View style={styles.imageOptionsContainer}>
            <TouchableOpacity style={[styles.imageOption, { backgroundColor: currentTheme.background, borderColor: currentTheme.border }]} onPress={selectImage}>
              <Ionicons name="images" size={32} color={currentTheme.primary} />
              <Text style={[styles.imageOptionText, { color: currentTheme.text }]}>Choose from Gallery</Text>
            </TouchableOpacity>
            
            <TouchableOpacity style={[styles.imageOption, { backgroundColor: currentTheme.background, borderColor: currentTheme.border }]} onPress={takePhoto}>
              <Ionicons name="camera" size={32} color={currentTheme.primary} />
              <Text style={[styles.imageOptionText, { color: currentTheme.text }]}>Take Photo</Text>
            </TouchableOpacity>
            
            <TouchableOpacity 
              style={[styles.imageOption, { backgroundColor: currentTheme.background, borderColor: currentTheme.border }]} 
              onPress={() => {
                setSelectedImage(null);
                setShowImageOptions(false);
              }}
            >
              <Ionicons name="sparkles" size={32} color={currentTheme.accent} />
              <Text style={[styles.imageOptionText, { color: currentTheme.text }]}>AI Generated</Text>
              <Text style={[styles.imageOptionSubtext, { color: currentTheme.textSecondary }]}>Let AI create one for you</Text>
            </TouchableOpacity>
          </View>
          
          {selectedImage && (
            <View style={[styles.selectedImageContainer, { backgroundColor: currentTheme.background }]}>
              <Text style={[styles.selectedImageText, { color: currentTheme.text }]}>Selected Image:</Text>
              <Image source={{ uri: selectedImage }} style={styles.selectedImagePreview} />
              <TouchableOpacity 
                style={styles.removeImageButton}
                onPress={() => setSelectedImage(null)}
              >
                <Ionicons name="trash" size={20} color={currentTheme.error} />
                <Text style={[styles.removeImageText, { color: currentTheme.error }]}>Remove</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
  ...
  if (loading) {
    return (
      <SafeAreaView style={[styles.loadingContainer, { backgroundColor: currentTheme.background }]}>
        ...
        <Text style={[styles.loadingText, { color: currentTheme.text }]}>Generating your personalized workout...</Text>
      </SafeAreaView>
    );
  }
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: currentTheme.background }]}>
        <ScrollView contentContainerStyle={styles.welcomeContainer}>
          ...
          {sourceWorkout && (
            <View style={[styles.similarWorkoutBanner, { backgroundColor: currentTheme.primaryLight }]}>
              <Ionicons name="refresh" size={20} color={currentTheme.primary} />
              <Text style={[styles.similarWorkoutText, { color: currentTheme.primary }]}>
          ...
          <View style={styles.header}>
            <Ionicons name="fitness" size={60} color={currentTheme.primary} />
            <Text style={[styles.title, { color: currentTheme.text }]}>AI Workout Generator</Text>
            <Text style={[styles.subtitle, { color: currentTheme.textSecondary }]}>
              {userProfile?.role === 'group_member' 
                ? 'Get hockey-inspired fitness workouts instantly'
                : 'Get hockey-specific workouts instantly'
              }
            </Text>
          </View>

          <View style={styles.suggestionsContainer}>
            <Text style={[styles.suggestionsTitle, { color: currentTheme.text }]}>Quick Start Options:</Text>
            {suggestions.map((suggestion) => (
              ...
              >
                <Text style={[styles.suggestionTitle, { color: currentTheme.text }]}>{suggestion.title}</Text>
                <Text style={[styles.suggestionDescription, { color: currentTheme.textSecondary }]}>
                  {suggestion.description}
                </Text>
                <Text style={[styles.suggestionDuration, { color: currentTheme.textSecondary }]}>
                  Duration: {suggestion.duration}
                </Text>
              </TouchableOpacity>
            ))}
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: currentTheme.background }]}>
      <View style={[styles.workoutHeader, { backgroundColor: currentTheme.surface, borderBottomColor: currentTheme.border }]}>
        <View style={styles.workoutTitleContainer}>
          <Text style={[styles.workoutTitle, { color: currentTheme.text }]}>
            {workout.title}
          </Text>
          <Text style={[styles.workoutMeta, { color: currentTheme.textSecondary }]}>
            {workout.difficulty} • {workout.estimatedDuration}
          </Text>
        </View>
        <View style={styles.progressContainer}>
          <Text style={[styles.progressText, { color: currentTheme.primary }]}>{calculateProgress()}%</Text>
          <Text style={[styles.progressLabel, { color: currentTheme.textSecondary }]}>Complete</Text>
        </View>
      </View>

      <ScrollView style={[styles.workoutContent, { backgroundColor: currentTheme.background }]}>
      ...
      <View style={[styles.bottomActions, { backgroundColor: currentTheme.surface, borderTopColor: currentTheme.border }]}>
        <TouchableOpacity
          style={[styles.newWorkoutButton, { backgroundColor: currentTheme.surface, borderColor: currentTheme.primary }]}
          onPress={() => {
            setIsQuickStartMode(false); // Reset quick start mode
            setWorkout(null);
          }}
        >
          <Text style={[styles.newWorkoutButtonText, { color: currentTheme.primary }]}>New Workout</Text>
        </TouchableOpacity>
        ...
        >
          {saving ? (
            <View style={styles.savingContainer}>
              ...
              <Text style={[styles.savingText, { color: currentTheme.surface }]}>Saving...</Text>
            </View>
          ) : (
            <Text style={[styles.completeButtonText, { color: currentTheme.surface }]}>Complete Workout</Text>
          )}
        </TouchableOpacity>
      </View>
    </SafeAreaView>
