...
import { updateCustomTeamTheme } from '../services/theme';

const CustomThemeSelector = ({ visible, onClose, onSave, onBack }) => {
  console.log('🎨 CustomThemeSelector rendered, visible:', visible);
  ...
  const handleSave = async () => {
    console.log('🎨 Attempting to save custom theme...');
    ...
    if (!userProfile?.teamId) {
      Alert.alert('Error', 'No team found. Please make sure you are part of a team.');
      return;
    }

    if (userProfile?.role !== 'coach') {
      Alert.alert('Permission Denied', 'Only coaches can customize team themes.');
      return;
    }

    setLoading(true);
    try {
      console.log('🎨 Saving custom theme for team:', userProfile.teamId);
      await updateCustomTeamTheme(userProfile.teamId, {
        name: 'Custom',
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet">
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Custom Team Colors</Text>
          <TouchableOpacity 
            onPress={handleSave} 
