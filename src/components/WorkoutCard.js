...
const WorkoutCard = ({ 
  ...
  return (
    <TouchableOpacity 
      style={[styles.card, { backgroundColor: currentTheme.surface || currentTheme.card || '#FFFFFF' }, style]} 
      onPress={onPress}
      activeOpacity={0.7}
    >
        ...
        <View style={styles.workoutInfo}>
          {showPlayerName && workout.userName && (
            <TouchableOpacity 
              ...
              activeOpacity={0.7}
            >
              <Text style={[styles.playerName, { color: currentTheme.primary }]}>{workout.userName}</Text>
          ...
          <View style={styles.typeContainer}>
            <Text style={[styles.workoutType, { color: currentTheme.text || '#000000' }]}>{workout.type}</Text>
          ...
          <Text style={[styles.duration, { color: currentTheme.textSecondary || '#8E8E93' }]}>{workout.duration} minutes</Text>
        </View>
        <Text style={[styles.timestamp, { color: currentTheme.textSecondary || '#8E8E93' }]}>{formatDate(workout.timestamp)}</Text>
      </View>

      {workout.notes && (
        <Text style={[styles.notes, { color: currentTheme.text || '#333333' }]} numberOfLines={3}>
          {workout.notes}
        </Text>
      )}
      ...
      {imageLoadError && workout.hasAIImage && (
        <View style={styles.imageErrorContainer}>
          ...
          <Text style={[styles.imageErrorText, { color: currentTheme.textSecondary || '#8E8E93' }]}>AI image expired</Text>
          <Text style={[styles.imageErrorSubtext, { color: currentTheme.textMuted || '#C7C7CC' }]}>This image was temporarily generated and is no longer available</Text>
        </View>
      )}

      <View style={[styles.footer, { borderTopColor: currentTheme.border || '#F2F2F7' }]}>
        <TouchableOpacity 
          style={[styles.likeButton, { backgroundColor: currentTheme.buttonBackground || '#F8F9FA' }]}
        ...
        >
          ...
          <Text style={[
            styles.likeCount,
            { color: userHasLiked ? currentTheme.primary : (currentTheme.textSecondary || '#8E8E93') }
          ]}>
            {localLikeCount}
          </Text>
        </TouchableOpacity>
        ...
        {workout.commentCount > 0 && (
          <View style={styles.commentsIndicator}>
            ...
            <Text style={[styles.commentCount, { color: currentTheme.textSecondary || '#8E8E93' }]}>{workout.commentCount}</Text>
          </View>
        )}
