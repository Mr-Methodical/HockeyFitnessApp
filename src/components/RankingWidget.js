...
const RankingWidget = ({ navigation }) => {
  ...
  const getRankColor = (position) => {
    if (!position) return currentTheme.textSecondary || '#666';
    if (position === 1) return '#FFD700';
    if (position === 2) return '#C0C0C0';
    if (position === 3) return '#CD7F32';
    return currentTheme.primary || '#007AFF';
  };
    ...
    return (
      <View style={[styles.container, { backgroundColor: currentTheme.surface }]}>
        <View style={styles.loadingContainer}>
          ...
          <Text style={[styles.loadingText, { color: currentTheme.textSecondary }]}>Loading ranking...</Text>
        </View>
      </View>
    );
  }
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: currentTheme.text }]}>My Ranking</Text>
        <MaterialIcons name="chevron-right" size={20} color={currentTheme.textSecondary || '#ccc'} />
      </View>
      ...
      <View style={styles.content}>
        <View style={styles.rankSection}>
          ...
          <View style={styles.rankInfo}>
            <Text style={[styles.rankText, { color: currentTheme.textSecondary }]}>
              {ranking?.position ? `${ranking.position} of ${ranking.totalPlayers}` : 'Unranked'}
            </Text>
            {ranking?.isTopThree && (
              <Text style={[styles.topThreeText, { color: currentTheme.primary }]}>🎉 Top 3!</Text>
            )}
          </View>
        </View>
        ...
        {playerScore && (
          <View style={styles.scoreSection}>
            <View style={styles.scoreItem}>
              <Text style={[styles.scoreValue, { color: currentTheme.text }]}>{playerScore.totalScore}</Text>
              <Text style={[styles.scoreLabel, { color: currentTheme.textSecondary }]}>Score</Text>
            </View>
