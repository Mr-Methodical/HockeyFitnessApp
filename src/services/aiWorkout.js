...
import { generateWorkoutSecure, generateWorkoutImageSecure, checkBackendHealth } from './backendApi';
...
const generateWorkoutDirect = async (preferences = {}, userRole = null) => {
  try {
    ...
    if (userRole === 'group_member') {
      ...
      systemPrompt = `You are a professional fitness trainer creating personalized workouts for general fitness enthusiasts. Create engaging, safe, and effective workouts that focus on overall health and fitness. NEVER mention hockey, ice sports, skating, or any sport-specific terms.`;
      
      userPrompt = `Create a ${duration}-minute ${intensity} intensity workout focused on ${focus}. Equipment available: ${equipment}.
      
      IMPORTANT: This is for general fitness training - do NOT include any hockey, ice, skating, or sport-specific references in the title, description, or exercises.
      
      Return a JSON object with this exact structure:
      {
        "title": "General fitness workout name (no hockey references)",
        "description": "Brief description of general fitness benefits",
...
const uploadWorkoutImage = async (imageUri, userId) => {
  try {
    console.log('📤 Uploading user workout image to Firebase Storage...');
    ...
    const imageName = `user_workout_${timestamp}.jpg`;
    ...
    console.log('✅ User image uploaded successfully:', downloadURL);
...
const uploadWorkoutImageAlternative = async (imageUri, userId) => {
  try {
    console.log('📤 Trying alternative upload method using different approach...');
    const { ref, uploadBytesResumable, getDownloadURL } = await import('firebase/storage');
    const { storage } = await import('./firebase');
    
    // Create unique filename
    const timestamp = Date.now();
    const imageName = `user_workout_alt_${timestamp}.jpg`;
    const imageRef = ref(storage, `workouts/${userId}/${imageName}`);
    
    // For React Native, we can use XMLHttpRequest approach
    ...
    console.log('📤 Alternative blob created, size:', blob.size, 'bytes');
      ...
      uploadTask.on('state_changed',
        ...
        async () => {
          try {
            ...
            console.log('✅ User image uploaded successfully (alternative):', downloadURL);
            resolve(downloadURL);
          } catch (urlError) {
            reject(urlError);
          }
...
const saveAIWorkout = async (userId, workoutData, totalTime, teamId = null, userImageUri = null, userRole = null) => {
  try {
    ...
    if (userImageUri === 'no-image') {
      ...
      // Check if this is already a valid stock image URL from fast generation
      if (userImageUri.includes('unsplash.com') || userImageUri.includes('https://')) {
        ...
        // User provided a local image - upload it
        try {
          ...
          try {
            ...
            console.error('❌ Alternative upload also failed, proceeding without image:', altUploadError);
            console.log('📸 User image upload failed completely - no image will be added');
            finalImageUrl = null;
          }
        }
      }
